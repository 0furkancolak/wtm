...
test.skipIf(process.platform !== 'linux' || !['x64', 'arm64'].includes(process.arch))(
  'the native Linux archive extracts exact bytes, executable mode, notices and verified checksums',
  async () => {
    const result = await runNativeArchiveScenario();

    expect(result).toMatchObject({ platform: 'linux', arch: process.arch, inputExecutable: '/usr/bin/true',
      archiveName: `wtm-linux-${process.arch}.tar.gz`, mode: 0o755, smokeExitCode: 0 });
